    ...
    [
      {
        ...
        task: childTrace<Parameters<ForgeListrTaskFn<PackageContext>>>({ name: 'package-prepare', category: '@electron-forge/api' }, async (_, ctx) => {
      ...
      },
      {
        ...
        task: childTrace<Parameters<ForgeListrTaskFn<PackageContext>>>(
          { name: 'run-packaging-hooks', category: '@electron-forge/api' },
              ...
              task.newListr([
                {
                  ...
                  task: childTrace<Parameters<ForgeListrTaskFn>>(
                    { name: 'run-generateAssets-hook', category: '@electron-forge/api' },
                ...
                },
                {
                  ...
                  task: childTrace<Parameters<ForgeListrTaskFn>>(
                    { name: 'run-prePackage-hook', category: '@electron-forge/api' },
                    async (childTrace, _, task) => {
                      return delayTraceTillSignal(
                        childTrace,
      ...
      },
      {
        ...
        task: childTrace<Parameters<ForgeListrTaskFn<PackageContext>>>(
          { name: 'packaging-application', category: '@electron-forge/api' },
                              ...
                              return delayTraceTillSignal(
                                ...
                                task.newListr(
                                  [
                                    {
                                      title: 'Copying files',
                                      task: childTrace({ name: 'copy-files', category: '@electron-forge/api' }, async () => {
                                        await addSignalAndWait(signalCopyDone, target);
                                      }),
                                    },
                                    {
                                      title: 'Preparing native dependencies',
                                      task: childTrace({ name: 'prepare-native-dependencies', category: '@electron-forge/api' }, async (_, __, task) => {
                                    ...
                                    },
                                    {
                                      ...
                                      task: childTrace({ name: 'finalize-package', category: '@electron-forge/api' }, async () => {
                                        await addSignalAndWait(signalPackageDone, target);
                                      }),
                                    },
                                  ],
      ...
      },
      {
        ...
        task: childTrace<Parameters<ForgeListrTaskFn<PackageContext>>>(
          { name: 'run-postPackage-hook', category: '@electron-forge/api' },
...
export default autoTrace({ name: 'package()', category: '@electron-forge/api' }, async (childTrace, opts: PackageOptions): Promise<PackageResult[]> => {
