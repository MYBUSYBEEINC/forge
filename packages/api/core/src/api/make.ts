  ...
  const runner = new Listr<MakeContext>(
    [
      ...
      },
      {
        ...
        task: childTrace<Parameters<ForgeListrTaskFn<MakeContext>>>(
          { name: 'run-preMake-hook', category: '@electron-forge/api' },
            ...
            for (const targetArch of parseArchs(platform, arch, await getElectronVersion(dir, packageJSON))) {
              ...
              for (const maker of makers) {
                subRunner.add({
                  ...
                  task: childTrace<[]>({ name: `make-${maker.name}`, category: '@electron-forge/core' }, async () => {
                    try {
                      /**
      ...
      },
      {
        ...
        task: childTrace<Parameters<ForgeListrTaskFn<MakeContext>>>({ name: 'run-postMake-hook', category: '@electron-forge/api' }, async (_, ctx, task) => {
